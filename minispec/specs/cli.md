...
- `minspect uninstall [--agent <a>] [--all] [--purge] [--yes]`（卡 41）：对称 install。默认 dry-run 只打印"will remove"；`--yes` 才写。`--agent claude-code` 只撤 settings.json 里 `__minspect_managed__: true` 的 hook entry；`--agent opencode` 若文件整份是我们的就删文件，否则只 strip BEGIN/END 块保留用户代码；`--all` 额外撤 cwd git 仓 post-commit + **撤 autostart**（在 stop-daemon 之前，确保 unit 文件先于 daemon 停止被清掉）+ 停 daemon；`--purge` 再删 `<state_dir>/history.sqlite*` + `sessions/` + `queue/`。全部步骤每项单独备份 `.bak.<ts>`。
- `minspect doctor [--json]`（卡 42）：诊断输出，**9 项**检查：`node`（≥20）、`state-dir`（可写）、`daemon`（state.json 正确且 /health 200）、`hook-claude-code`、`hook-opencode`、`hook-post-commit`（非 git 仓 skip）、**`autostart`**（按 `process.platform` 查对应 unit 文件 / 任务是否存在 + 与 config 一致；缺/坏则 ⚠+fix 建议）、`db`（history.sqlite 存在）、`events`（/api/sessions 最近 5 min 内有活动）。每项 ✓/⚠/✗ + fix 建议；`--json` 机器可读；有 fail 退出码 1。
- `minspect status [--json]`（卡 45）：只读诊断输出，daemon 状态 / 端口 / pid / `spawned_by` / queue / 最近 event age / hook 安装状态 / **autostart 摘要**（enabled + unit 是否存在 + backend + unit path）。**无参 `minspect` 默认调用 status**（commander `isDefault: true`），帮助页仍由 `--help` 触发。
- `minspect init [--yes]`（卡 44 + 卡 48 + 卡 49）：聚合命令。流程：跑 doctor → 检测 agent（`~/.claude/settings.json` / `~/.config/opencode/` / `~/.codex/sessions/`） → 逐项交互问是否装 Claude Code hook / OpenCode 插件 / Codex 30d import / cwd git 仓 post-commit hook → 首次问 `auto_spawn_daemon`（写入 `<state_dir>/config.json`） → **首次问 `autostart`**（与 auto_spawn 独立；非交互 `--yes` 默认 `true`——理由：注册是用户可见可逆的，不像 auto_spawn 偷偷 spawn 进程那么隐式） → **detach-spawn 后台 daemon**（`spawnServeDetached({spawnedBy: 'init'})`） → `waitForDaemonReady` 最多 5s 轮询 `/health` → 打开浏览器 → init 本身 exit 0。init 完成后可关闭终端，daemon 不受影响。若 daemon 已在跑，`findRunningDaemon` 命中直接打印 `already running on :PORT` + 开浏览器，不重复 spawn。spawn 超时走"请手动跑 `minspect serve` 看错误"的降级路径，不崩溃。`--yes` 非交互，保守默认（装检测到的 agent hook、post-commit if git、不开 auto_spawn、**开 autostart**、不导入 Codex 避免长耗时）。已装 hook 自动 skip；重复 init 幂等。
- `minspect start [--open]`（卡 50 补充）：**只**做 detach-spawn 后台 daemon，不跑 doctor、不装 hook、不动 config / autostart、不打开浏览器。daemon 已跑 → 复用现有 daemon（`findRunningDaemon` 命中后直接 print `already running on :PORT`）；daemon 不在 → spawn + 5s `/health` 轮询 + 打印 `daemon: http://127.0.0.1:PORT (pid N)`。失败时（spawn 返回 null 或 5s 超时）→ stderr + exit 1，提示手动 `minspect serve` 看错误。`--open` 额外开浏览器（默认不开）。使用场景：用户感知 daemon 挂了但不想再跑 init 那一堆；替代 `serve`（前台占 terminal）的快速恢复路径。内部委托给 `init.ts` 抽出的 `runStartDaemonDetached({spawnedBy: 'user'})`，保证 init 末尾 spawn 块和 standalone `start` 走完全相同的代码路径。
- `minspect install-autostart [--backend <launchd|systemd|xdg-autostart|scheduled-task>] [--dry-run]`（卡 49；Windows 形态由卡 `20260831-fix-autostart-console-window` 修订）：把 daemon 注册为用户登录启动项。平台默认按 `process.platform` 选 backend：macOS → `~/Library/LaunchAgents/com.ivenlau.minspect.plist`（`launchctl bootstrap gui/$UID`）；Linux → `~/.config/systemd/user/minspect.service`（`systemctl --user enable --now`），systemd 不可用时自动降级到 `~/.config/autostart/minspect.desktop`；Windows → HKCU Run key（`reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v minspect-daemon /t REG_SZ /d "wscript.exe \"<state_dir>\minspect-daemon.vbs\"" /f`）。Run 值**不直接指向 node.exe**——node 是控制台子系统程序，Run 键裸启会在登录时弹出可见控制台窗口，窗口被关闭会连带杀掉 daemon（真机取证：daemon 在登录后数分钟消失、无崩溃记录）。改为生成 `<state_dir>/minspect-daemon.vbs`（UTF-16 LE + BOM 写盘，WSH 按 BOM 判编码、不依赖系统 codepage），内容为 `CreateObject("WScript.Shell").Run "<node> <bin> serve --quiet", 0, False`：窗口样式 0（隐藏）+ 不等待，对齐 `spawnServeDetached` 的 detached + windowsHide 语义。VBS 内 node/bin 用**绝对路径**（`process.execPath` + `process.argv[1]`，不依赖 PATH）且**原样写入、不做 `\`→`\\` 双写**——reg.exe 经 argv 收到的 /d 数据是字面存储的，旧 escapeReg 的双写只是碰巧被 Win32 路径归一化容忍；VBS 字符串字面量内的 `"` 按 VBScript 规则双写。`wscript.exe` 用裸名（登录会话必能从 System32 解析）。Windows 走 HKCU Run key 是因为非 admin 用户创建 ONLOGON 任务会被 `schtasks` 拒绝；Run key 是用户态、Explorer 在 logon 时 launch，与 ONLOGON 同 UX。值名 `minspect-daemon`（连字符而非空格）避免未来任何 tokenizing 路径再踩坑。所有三平台都设 `KeepAlive.SuccessfulExit=false` / `Restart=on-failure`（区别于崩溃重启），保证 `minspect stop`（exit 0）不被立即拉起。注册成功后写 `autostart: true` 到 `<state_dir>/config.json`。`--dry-run` 只打印计划、不写文件、不调 OS 命令。`--backend` 强制指定（测试 / 跨平台脚本用）。`code` 内 backend 仍叫 `scheduled-task`（历史命名；不再用 Task Scheduler，但 `unitPath` 用 `HKCU\…\Run\<name>` 字符串、status/doctor 直接 shell 出 `reg query` 探针，不再乐观撒谎）。已装 2026-08-31 之前形态的机器重跑 `install-autostart`（或 `init`）即迁移。
- `minspect uninstall-autostart [--backend <...>] [--yes]`（卡 49；Windows 清理由卡 `20260831-fix-autostart-console-window` 扩展）：撤销登录启动项。默认 dry-run；`--yes` 真正执行（`launchctl bootout` / `systemctl --user disable --now` / `reg delete HKCU\…\Run /v minspect-daemon /f`；Windows 另删除 `<state_dir>/minspect-daemon.vbs`，缺文件静默跳过）。成功后会写 `autostart: false` 到 config。
- `minspect revert --turn <id> | --edit <id> [--yes] [--force]`：把文件恢复到 turn/edit 之前的状态。默认 dry-run；`--yes` 才写盘。drift 检测（磁盘当前文件 sha256 ≠ expected after_hash）默认拒绝，`--force` 覆盖。Codex 来源 session **硬拒绝**（hunk 窗口级 before/after 不足以安全 revert）。AI 新建的文件（before_hash=NULL）revert = 删除。核心逻辑（`checkDrift`、`applyRevert`、`sha256`）抽到 `@minspect/core` 共享，CLI 和 server 的 execute 端点复用同一条代码路径。
...
- `runInstallAutostart({stateRoot?, nodePath?, minspectBinPath?, backend?, persist?, dryRun?})`（卡 49）：返回 `{backend, unitPath, enabled, started, detail}`。`backend` 显式传入时跳过 platform() 自动检测；`persist:false` 不动 config（供 `init` 写自己的 config 时用）。`dryRun:true` 不写文件不调 OS 命令，只返回计划。
- `runUninstallAutostart({stateRoot?, nodePath?, minspectBinPath?, backend?, persist?, yes?})` / `planUninstallAutostartSteps({...})`（卡 49）：跟 `uninstall.ts` 对称；默认 dry-run（`yes:false`），`--yes` 才真撤销。`formatUninstallAutostartReport(result)` 给人看的报告。
- `hasSystemdUser()` / `launchdPlistPath()` / `systemdUnitPath()` / `xdgAutostartDesktopPath()` / `scheduledTaskName()`（卡 49）：platform backend 的纯函数 path/dispatch 工具，给 `status` / `doctor` 用，**不**调 OS 命令、不写文件。
...
- **autostart 用户态、无 sudo**（卡 49）：三平台都只用用户态的登录项机制（macOS LaunchAgent / Linux `systemd --user` + XDG 降级 / Windows HKCU Run key），不写 `/etc` `/Library/LaunchDaemons` `C:\Windows\System32`；不创建系统级 systemd unit / LaunchDaemon / 任务计划程序任务。`ProcessType=Interactive`、Linux `WantedBy=default.target`、Windows HKCU Run key 三选一保 daemon 跟用户会话同生命周期。Windows 之所以放弃 Task Scheduler：非 admin 用户 `schtasks /Create /SC ONLOGON` 必返回 "Access is denied"（ONLOGON 触发器需要 admin 串起 session-attached token），改用 HKCU Run key 后任何交互式用户都能写、Explorer 在 logon 时 launch，等同 ONLOGON 的 UX。
- **autostart unit 文件用绝对路径，但注册表数据字面存储**（卡 49；卡 `20260831-fix-autostart-console-window` 修正）：`ProgramArguments / ExecStart` 必须含 `process.execPath`（node 绝对路径）+ `process.argv[1]`（bin.js 绝对路径），不依赖 PATH——保证 systemd --user / launchd / Run key 在非交互 session 下也能找到二进制。Linux 在没有 systemd 时降级到 XDG autostart desktop 文件，再不行返回 `unsupported`。**Windows 的 `reg add /d` 数据是字面存储的**：reg.exe 不处理 `\` 转义，路径里的 `\` 不要双写成 `\\`（旧 escapeReg 的双写被原样存进注册表，仅因 Win32 路径归一化容忍重复分隔符才没炸；路径以 `\` 结尾时更会破坏收尾引号）。内嵌 `"` 的转义只发生在 VBS 字符串字面量内（VBScript 规则：`"` → `""`）。
- **Windows 登录启动不得裸启控制台程序**（卡 `20260831-fix-autostart-console-window`）：Run key 直接启动 node.exe 会在登录时弹出可见控制台窗口，窗口被关闭即杀死 daemon。必须经 `<state_dir>/minspect-daemon.vbs` 的 `WScript.Shell.Run "...", 0, False`（隐藏 + 不等待）包装，与 `spawnServeDetached` 的 `detached + windowsHide` 行为对齐。
- **autostart 单元重启策略与 `minspect stop` 不打架**（卡 49）：launchd `KeepAlive.SuccessfulExit=false` / `Crashed=true`；systemd `Restart=on-failure`；保证用户 `minspect stop`（exit 0）后 OS 不会立即把它拉回；只有异常崩溃才会被 OS 拉起。
...
- **Bash 造成的文件变更也要归因**（卡 `20260901-capture-bash-file-edits`）：auto mode 下 agent 常用 Bash（heredoc `cat >` / sed -i / tee）写文件，payload 没有 file_path，Edit/Write 工具的 pre/post 机制失效。capture 在 SessionStart 与每次 PostToolUse 后维护 `git status --porcelain` + dirty/untracked 内容缓存（cap 400 路径 / 12MB）的 worktree 快照（session state `bash_snapshot`），PostToolUse(Bash) diff 出变更路径：before 依次取缓存内容（脏文件链精确）→ `git cat-file --filters :path`（干净文件取 index 版本，EOL 对齐磁盘，避免 autocrlf 幻影全文件 diff）→ null（本次新建）；before==after 跳过；删除（v1）与非 git 仓不记，只落 tool_call。设计给 hook 的额外开销 ≈ 每次 tool call 一次 `git status` + 变更路径读盘。`install` 剥离 minspect hook 块时按标记 **或** command 含 `capture --event ` 匹配（旧格式无标记的块也要能清掉，否则每个工具调用 capture 跑两遍）。
