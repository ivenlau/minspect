...
- `minspect install-autostart [--backend <launchd|systemd|xdg-autostart|scheduled-task>] [--dry-run]`（卡 49）：把 daemon 注册为用户登录启动项。平台默认按 `process.platform` 选 backend：macOS → `~/Library/LaunchAgents/com.ivenlau.minspect.plist`（`launchctl bootstrap gui/$UID`）；Linux → `~/.config/systemd/user/minspect.service`（`systemctl --user enable --now`），systemd 不可用时自动降级到 `~/.config/autostart/minspect.desktop`；Windows → HKCU Run key（`reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v minspect-daemon /t REG_SZ /d "..." /f`）。`ProgramArguments / ExecStart / reg /d` 都用**绝对路径**（`process.execPath` + `process.argv[1]`，不依赖 PATH），路径里的 `\` 和 `"` 各自转义。Windows 走 HKCU Run key 是因为非 admin 用户创建 ONLOGON 任务会被 `schtasks` 拒绝；Run key 是用户态、Explorer 在 logon 时 launch，与 ONLOGON 同 UX。值名 `minspect-daemon`（连字符而非空格）避免未来任何 tokenizing 路径再踩坑。所有三平台都设 `KeepAlive.SuccessfulExit=false` / `Restart=on-failure`（区别于崩溃重启），保证 `minspect stop`（exit 0）不被立即拉起。注册成功后写 `autostart: true` 到 `<state_dir>/config.json`。`--dry-run` 只打印计划、不写文件、不调 OS 命令。`--backend` 强制指定（测试 / 跨平台脚本用）。`code` 内 backend 仍叫 `scheduled-task`（历史命名；不再用 Task Scheduler，但 `unitPath` 用 `HKCU\…\Run\<name>` 字符串、status/doctor 直接 shell 出 `reg query` 探针，不再乐观撒谎）。
- `minspect uninstall-autostart [--backend <...>] [--yes]`（卡 49）：撤销登录启动项。默认 dry-run；`--yes` 真正执行（`launchctl bootout` / `systemctl --user disable --now` / `reg delete HKCU\…\Run /v minspect-daemon /f`）。成功后会写 `autostart: false` 到 config。
...
- **autostart unit 文件用绝对路径**（卡 49）：`ProgramArguments / ExecStart / reg /d` 必须含 `process.execPath`（node 绝对路径）+ `process.argv[1]`（bin.js 绝对路径），不依赖 PATH——保证 systemd --user / launchd / Run key 在非交互 session 下也能找到二进制。Linux 在没有 systemd 时降级到 XDG autostart desktop 文件，再不行返回 `unsupported`。Windows Run key value 内的内嵌 `"` 要转义为 `\"`、`\` 要转义为 `\\`（reg.exe 把它当 escape），避免以后回归。
