...
- `minspect install-autostart [--backend <launchd|systemd|xdg-autostart|scheduled-task>] [--dry-run]`（卡 49）：把 daemon 注册为用户登录启动项。平台默认按 `process.platform` 选 backend：macOS → `~/Library/LaunchAgents/com.ivenlau.minspect.plist`（`launchctl bootstrap gui/$UID`）；Linux → `~/.config/systemd/user/minspect.service`（`systemctl --user enable --now`），systemd 不可用时自动降级到 `~/.config/autostart/minspect.desktop`；Windows → 任务计划程序 "minspect daemon"（`schtasks /Create /SC ONLOGON /RL LIMITED /F /IT`）。`ProgramArguments / ExecStart` 用**绝对路径**（`process.execPath` + `process.argv[1]`，不依赖 PATH）。所有三平台都设 `KeepAlive.SuccessfulExit=false` / `Restart=on-failure`（区别于崩溃重启），保证 `minspect stop`（exit 0）不被立即拉起。注册成功后写 `autostart: true` 到 `<state_dir>/config.json`。`--dry-run` 只打印计划、不写文件、不调 OS 命令。`--backend` 强制指定（测试 / 跨平台脚本用）。
- `minspect uninstall-autostart [--backend <...>] [--yes]`（卡 49）：撤销登录启动项。默认 dry-run；`--yes` 真正执行（`launchctl bootout` / `systemctl --user disable --now` / `schtasks /Delete`，并删文件）。成功后会写 `autostart: false` 到 config。
...
- **autostart 用户态、无 sudo**（卡 49）：三平台都只用用户态的登录项机制（macOS LaunchAgent / Linux `systemd --user` + XDG 降级 / Windows Task Scheduler `RL LIMITED`），不写 `/etc` `/Library/LaunchDaemons` `C:\Windows\System32`；不创建系统级 systemd unit / LaunchDaemon / ONSTART 任务。`ProcessType=Interactive`、task `RL LIMITED`、Linux `WantedBy=default.target` 三选一保 daemon 跟用户会话同生命周期。
- **autostart unit 文件用绝对路径**（卡 49）：`ProgramArguments / ExecStart` 必须含 `process.execPath`（node 绝对路径）+ `process.argv[1]`（bin.js 绝对路径），不依赖 PATH——保证 systemd --user / launchd / Task Scheduler 在非交互 session 下也能找到二进制。Linux 在没有 systemd 时降级到 XDG autostart desktop 文件，再不行返回 `unsupported`。
