...
- [x] Given 临时 git 仓中 PostToolUse(Bash, `cat > new.java`)，When capture，Then 产出 tool_call + file_edits（before=null → creation 语义），collector 侧可落 edits 行。（capture.test.ts + 真机端到端：隔离 LOCALAPPDATA 下真实 0.1.10 二进制 → edits 行 before_hash=null）
- [x] Given 已被上一轮 Bash 修改过的文件再次被 Bash 修改，When capture，Then before = 上一轮 after（内容缓存，精确）。（单测 + 真机端到端第二次 heredoc：before_hash 串起上一轮 after_hash）
- [x] Given 干净 tracked 文件被 Bash 改写，When capture，Then before = `git cat-file --filters :path`（index 版本，EOL 对齐磁盘）。（单测 base.txt 用例）
- [x] Given Bash 执行无文件变化的命令，When capture，Then 不产生 file_edits（before===after 或 status 无变化）。（单测 no-op 用例）
- [x] Given 非git 目录的 Bash PostToolUse，When capture，Then 只有 tool_call、无 file_edits、无异常。（单测 non-git 用例）
- [x] Given settings.json 中同事件挂了带标记 + 无标记两份 minspect hook，When `install --agent claude-code`，Then 重写后每个事件只剩一份。（install.test.ts 去重用例 + 真机重装后 5 个事件 ours=1，codegraph 的用户块保留）
- [x] `pnpm -r test` 全绿（Windows 181 tests + WSL Ubuntu 181 tests）；`pnpm lint` touched files 0 error；`pnpm -r build` 0 type error。
- [x] 真机：`minspect install --agent claude-code --scope user` 后 settings.json 无重复块；合成 heredoc payload 走真实 `minspect capture`（0.1.10 二进制 + 隔离 daemon + 隔离 sqlite）在 edits 表落两行（创建 + 链式改写）。

# Plan

- [x] T1 写 change card（本文件）
- [x] T2 `bash-edits.ts` 快照/diff/show 助手 + cap
- [x] T3 session-state 加 `bash_snapshot`；capture.ts 接线（SessionStart / PostToolUse）
- [x] T4 install.ts 去重 + install.test.ts 用例
- [x] T5 capture.test.ts 新场景
- [x] T6 `pnpm -r test` / lint / build
- [x] T7 真机验证（去重重装 + 合成 payload 端到端）
- [x] T8 close：更新 `minispec/specs/cli.md`，卡片归档
